...
public abstract class JdoRepository<T> implements Repository<T>{
  ...
  /**
   * @return the {@linkplain PersistenceManager} associated with the current {@linkplain Thread}.
   */
   ...
   * The ID number must exist, if not the {@link PersistenceManager} will throw a
  ...
  /**
   * As we have a Repository per Entity {@link JdoRepository} must be extended. This way the new
   * implementation has to indicate which Entity Class will be assigned to the new implementation of
   * the {@link JdoRepository}.
   * 
   ...
   * implementation which is use like a call back.
   * 
   * @param runnable
   *          the block of to execute.
   */
  ...
  /**
   * Perform a query and wrap the result into a {@link List}. This is useful to return the result of
   * a query to the view.
   * 
   * @param query
   *          to be executed.
   ...
   *          to be executed.
   * @param object
   *          which is a parameter for the query.
   * @return the wrapped result of the query.
   */
