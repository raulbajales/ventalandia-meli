...
public class QuestionServiceTest {
	...
	public void setup() {
		...
		persistenceManagerProvider = mock(PersistenceManagerProvider.class);

		questionRepository = new QuestionRepository(persistenceManagerProvider);
	...
	public void testGetQuestion() throws Exception {
		...
		when(meliService.getEntityFromMELI("/questions/" + questionId, com.ventalandia.meli.api.notification.Question.class)).thenReturn(QuestionHelper.create());
		when(itemService.getByMeliId("MLA430494065")).thenReturn(createItem());
		when(userService.getByMeliId(sellerId)).thenReturn(new User());
		when(persistenceManagerProvider.get()).thenReturn(persistenceManager);
